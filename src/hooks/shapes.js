...
export const DESKTOP_POINTS_PER_SHAPE = 1700;
export const MOBILE_POINTS_PER_SHAPE = 1700;
...
function generateDiamondPositions(count) {
  ...
  for (let i = 0; i < baseSides; i++) {
    ...
    const noiseR1 = 1 + Math.sin(i * 3.7) * 0.15 + Math.cos(i * 1.5) * 0.08;
...
function buildEdgesForDiamond(
  ...
  forEachNearbyPair(pos, searchRadius, (i, j) => {
    ...
    if (segI < fillSegId && segJ < fillSegId && segI !== segJ) {
      return;
    }
...
function generateDiamondPositionsMobile(count) {
  ...
  for (let s = 0; s < diamondSegments.length; s++) {
    ...
    const ptsForSeg = isLast
      ? edgeCount - currentPoint
      : Math.floor((lengths[s] / totalLength) * edgeCount);
...
// ============================================================
// 4.2. ESFERA — MOBILE EDGES
// ============================================================

function buildEdgesForSphereMobile(pos, maxDist = 0.48) {
  return buildEdgesForShape(pos, maxDist);
}
...
// 5.1. INFINITO — MOBILE
//
// !!! AGORA É EXATAMENTE O MESMO DO DESKTOP !!!
//
// A função continua separada para preservar a estrutura
// mobile/desktop, mas utiliza a mesma geração de posições.
// ============================================================

function generateInfinityPositionsMobile(count) {
  return generateInfinityPositions(count);
}

// ============================================================
// 5.2. INFINITO — MOBILE EDGES
//
// Também usa exatamente a mesma lógica geométrica do desktop.
// ============================================================

function buildEdgesForInfinityMobile(pos, maxDist = 0.35) {
  return buildEdgesForShape(pos, maxDist);
}
...
function generateEnvelopePositions(count) {
  ...
  const topLeft = [-1.8, 1.1, 0];

  const topRight = [1.8, 1.1, 0];

  const bottomRight = [1.8, -1.1, 0];

  const bottomLeft = [-1.8, -1.1, 0];
...
export function buildAllShapes(
  ...
  if (isMobile) {
    ...
    const edge3 = buildEdgesForSphereMobile(shape3, 0.48);

    // --------------------------------------------------------
    // INFINITO
    //
    // AGORA USA EXATAMENTE A MESMA
    // GEOMETRIA DO DESKTOP.
    // --------------------------------------------------------

    const shape4 = generateInfinityPositionsMobile(count);

    const edge4 = buildEdgesForInfinityMobile(shape4, 0.35);
